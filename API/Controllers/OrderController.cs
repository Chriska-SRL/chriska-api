...
namespace API.Controllers
{
    ...
    public class OrderController : ControllerBase
    {
        ...
        [HttpPost]
        [Authorize(Policy = nameof(Permission.CREATE_ORDERS))]
        public async Task<ActionResult<OrderResponse>> AddOrderAsync([FromBody] OrderAddRequest request)
        {
            request.setUserId(_tokenUtils.GetUserId());
            var result = await _facade.AddOrderAsync(request);
        ...
        [HttpDelete("{id}")]
        [Authorize(Policy = nameof(Permission.DELETE_ORDERS))]
        public async Task<IActionResult> DeleteOrderAsync(int id)
        {
            var request = new DeleteRequest(id);
            request.setUserId(_tokenUtils.GetUserId());
            await _facade.DeleteOrderAsync(request);
        ...
        [Authorize(Policy = nameof(Permission.EDIT_ORDERS))]
        public async Task<ActionResult<OrderResponse>> ChangeStatusOrderAsync(int id, OrderChangeStatusRequest request)
        {
