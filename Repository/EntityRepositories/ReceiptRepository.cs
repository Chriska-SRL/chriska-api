    ...
    public class ReceiptRepository : Repository<Receipt, Receipt.UpdatableData>, IReceiptRepository
    {
        ...
        public async Task<Receipt> AddAsync(Receipt receipt)
        {
            int newId = await ExecuteWriteWithAuditAsync(
                @"INSERT INTO Receipts (Date, Amount, Notes, ClientId)
                  OUTPUT INSERTED.Id VALUES (@Date, @Amount, @Notes, @ClientId)",
                receipt,
                AuditAction.Insert,
            ...
            return new Receipt(newId, receipt.Date, receipt.Amount, receipt.Notes, receipt.Client, receipt.AuditInfo);
        }
        ...
        public async Task<Receipt> UpdateAsync(Receipt receipt)
        {
            int rows = await ExecuteWriteWithAuditAsync(
                "UPDATE Receipts SET Notes = @Notes WHERE Id = @Id",
                receipt,
                AuditAction.Update,
                configureCommand: cmd =>
                {
        ...
        {
            var allowedFilters = new[] { "Date", "ClientId" };
