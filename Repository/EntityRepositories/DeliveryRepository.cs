    ...
    ";
        ...
        public async Task<Delivery> AddAsync(Delivery delivery)
        {
            int newId = await ExecuteWriteWithAuditAsync(
                @"INSERT INTO Deliveries (ClientId, Observations, Crates, OrderId)
                  OUTPUT INSERTED.Id VALUES (@ClientId, @Observations, @Crates, @OrderId)",
                delivery,
                AuditAction.Insert,
                configureCommand: cmd =>
                {
                    cmd.Parameters.AddWithValue("@ClientId", delivery.Client.Id);
